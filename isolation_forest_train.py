...
# Shuffle dataset first (important for randomness)
...
# Remove label column if it is accidentally numeric
...
# 6. TRAIN ONLY ON NORMAL TRAFFIC
# =========================================================
print("\n[INFO] Preparing Normal Training Data...")

normal_df = df[df[label_col] == "normal"]
X_train = normal_df[X.columns]

print(f"[INFO] Normal Samples Used: {len(X_train)}")
...
# RobustScaler is used because it handles outliers well
scaler = RobustScaler()
...
iso = IsolationForest(
    n_estimators=400,
    contamination=0.12,   # expected anomaly rate in real deployment
    max_samples='auto',
    random_state=42,
    n_jobs=-1
)
...
# 12. SAVE RESULTS
# =========================================================
OUTPUT_FILE = "models/isolation_forest_results.csv"
df.to_csv(OUTPUT_FILE, index=False)
...
print(f"Total Samples     : {len(df)}")
print(f"Actual Normals    : {(y_true == 0).sum()}")
print(f"Actual Attacks    : {(y_true == 1).sum()}")
